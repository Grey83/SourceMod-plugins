// Старая версия
// последняя правка от 01.08.2016
#pragma semicolon 1
...
Handle BuildMoveMenu()
{
	...
	char buffer[64];
	Format(buffer, sizeof(buffer), "Distance for move:", cAxis[iMAxis]);	// Сделать перевод
//	Format(buffer, sizeof(buffer), "Distance for move:\nAxis '%s'\n ", cAxis[iMAxis]);	// Сделать перевод
	...
	movemenu.AddItem("3", "-\n ");
	movemenu.AddItem("4", "+\n     0.25");
	movemenu.AddItem("5", "-\n \nAxis for move:");	// Сделать перевод

	for(int i; i < 3; i++)
	{
		Format(buffer, sizeof(buffer), "%d", i + 6);
		movemenu.AddItem(buffer, cAxis[i]);
	}
...
public int Menu_Move(Menu movemenu, MenuAction action, int client, int param2)
{
	switch(action)
	{
		...
		case MenuAction_DisplayItem:
		{
			...
			{
				char buffer[64];
				Format(buffer, sizeof(buffer), "%s %s", cAxis[param2 - 6], (param2 - 6 == iMAxis) ? "☑" : "	");
				return RedrawMenuItem(buffer);
//(param2 - 6 == iMAxis) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT;
			}
		}
		case MenuAction_Select:
		{
			if(0 <= param2 < 6)
			{
				...
				if(ent > MaxClients && IsValidEntity(ent))
				{
					...
					if(StrContains(ClassName, "npc_nmrih_", true) != 0)
					{
						...
						switch(param2)
						{
							case 0: pos[iMAxis] += 48;
							case 1: pos[iMAxis] -= 48;
							case 2: pos[iMAxis] += 16;
							case 3: pos[iMAxis] -= 16;
							case 4: pos[iMAxis] += 4;
							case 5: pos[iMAxis] -= 4;
						}
			...
			else if(5 < param2 < 9) iMAxis = param2 - 6;
